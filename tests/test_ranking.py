...
class RankingTests(unittest.TestCase):
    ...
    def test_tiered_strategy_prioritizes_kana_only_cards(self) -> None:
        cards = [
            self.make_card(
                card_id=1,
                expression="ありがとう",
        ...
        self.assertEqual([entry.card.card_id for entry in scored], [1, 2])
        self.assertEqual(scored[0].priority_label, "kana_only")
    ...
    def test_tiered_strategy_honors_custom_tier_order(self) -> None:
        ...
        scored = score_cards(cards, tier_order=custom_tier_order)
        self.assertEqual([entry.card.card_id for entry in scored], [2, 1])
    ...
    def test_tiered_strategy_can_disable_shorter_expression_tiebreak(self) -> None:
        ...
        scored = score_cards(cards, prefer_shorter_expressions=False)
        self.assertEqual([entry.card.card_id for entry in scored], [1, 2])
